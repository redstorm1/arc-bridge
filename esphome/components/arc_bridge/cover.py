...
CONFIG_SCHEMA = cover.cover_schema(ARCCover).extend(
    {
        cv.GenerateID(): cv.declare_id_with_type(ARCCover),
        cv.Required(CONF_BRIDGE_ID): cv.use_id_with_type(ARCBridgeComponent),
        cv.Required(CONF_BLIND_ID): cv.string,
        cv.Optional(CONF_LINK_QUALITY): cv.use_id_with_type(sensor.Sensor),
        cv.Optional(CONF_STATUS): cv.use_id_with_type(text_sensor.TextSensor),
