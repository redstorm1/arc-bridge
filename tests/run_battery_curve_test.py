...
def main() -> None:
    ...
    test_cpp = repo_root / "tests" / "battery_curve_test.cpp"
    battery_cpp = component_dir / "battery.cpp"

    compiler = find_compiler()
    std_flag = find_std_flag(compiler, repo_root)
    with tempfile.TemporaryDirectory() as tmpdir:
        binary = Path(tmpdir) / ("battery_curve_test.exe" if os.name == "nt" else "battery_curve_test")
        cmd = [
            compiler,
            std_flag,
            "-Wall",
