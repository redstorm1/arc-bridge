...
def main() -> None:
    ...
    test_cpp = repo_root / "tests" / "tx_queue_test.cpp"
    tx_queue_cpp = component_dir / "tx_queue.cpp"

    compiler = find_compiler()
    std_flag = find_std_flag(compiler, repo_root)
    with tempfile.TemporaryDirectory() as tmpdir:
        binary = Path(tmpdir) / ("tx_queue_test.exe" if os.name == "nt" else "tx_queue_test")
        cmd = [
            compiler,
            std_flag,
            "-Wall",
